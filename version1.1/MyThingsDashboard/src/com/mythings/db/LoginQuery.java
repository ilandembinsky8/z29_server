...
public class LoginQuery {
	...
	public static boolean checkID(Connection con, String id) throws NumberFormatException, SQLException{
		...
		rs = st.executeQuery("SELECT * FROM MyThingsDB.Users where id="+ Integer.parseInt(id));
		return rs.next();
	}
	...
	public static boolean checkPass(Connection con, String id, String pass) throws SQLException{
		...
		st =  con.createStatement();
		rs = st.executeQuery("SELECT * FROM MyThingsDB.Users where id="+ Integer.parseInt(id) + " AND pass='" + pass + "';");
		return rs.next();
	}
